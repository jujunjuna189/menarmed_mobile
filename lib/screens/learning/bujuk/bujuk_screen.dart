...
import 'package:menarmed_mobile/repository/bujuk_repo.dart';
import 'package:menarmed_mobile/widgets/shimmer/shimmer_widget.dart';
...
class _BujukScreenState extends State<BujukScreen> {
  ...
  Widget buildELearning() {
    return ListView(
      ...
      children: _bujuk.isNotEmpty ? _bujuk.map((value) {
        return Container(
          ...
          child: Column(
            ...
            children: [
              GestureDetector(
                  onTap: ((){
                    Navigator.of(context).pushNamed("/bujuk_view", arguments: jsonEncode({'path': value['path']}));
                  }),
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Row(
                        children: const [
                          Text("Menarmed Mobile", style: TextStyle(fontSize: 10),),
                          SizedBox(width: 5,),
